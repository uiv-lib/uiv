...
describe('ProgressBar', () => {
  ...
  it('should be able to render custom label', async () => {
    ...
    await vm.$nextTick()
    const bars = vm.$el.querySelectorAll('.progress')
    expect(bars[0].querySelector('.progress-bar').textContent).to.contain('Loading...')
  })
  ...
  it('should be able to render label with min width', async () => {
    ...
    await vm.$nextTick()
    const bars = vm.$el.querySelectorAll('.progress')
    expect(bars[0].querySelector('.progress-bar').style.minWidth).to.equal('2em')
  })
  ...
  it('should be able to render different types', async () => {
    ...
    await vm.$nextTick()
    const bars = vm.$el.querySelectorAll('.progress')
    expect(bars[0].querySelector('.progress-bar').className).to.contain('progress-bar-success')
    expect(bars[1].querySelector('.progress-bar').className).to.contain('progress-bar-info')
    expect(bars[2].querySelector('.progress-bar').className).to.contain('progress-bar-warning')
    expect(bars[3].querySelector('.progress-bar').className).to.contain('progress-bar-danger')
  })
  ...
  it('should be able to render striped', async () => {
    ...
    await vm.$nextTick()
    const bars = vm.$el.querySelectorAll('.progress')
    expect(bars[0].querySelector('.progress-bar').className).to.contain('progress-bar-striped')
    expect(bars[1].querySelector('.progress-bar').className).to.contain('progress-bar-striped')
    expect(bars[2].querySelector('.progress-bar').className).to.contain('progress-bar-striped')
    expect(bars[3].querySelector('.progress-bar').className).to.contain('progress-bar-striped')
  })
  ...
  it('should be able to render animation', async () => {
    ...
    await vm.$nextTick()
    const bars = vm.$el.querySelectorAll('.progress')
    expect(bars[0].querySelector('.progress-bar').className).to.contain('active')
  })

  it('should be able to render stacked', async () => {
    vm = createVm(`<div><progress-bar>
